...
def load_data_from_influxdb():
    """Load data from InfluxDB with sampling for faster training"""
    print("📡 Connecting to InfluxDB...")
    client = InfluxDBClient(url=INFLUX_URL, token=INFLUX_TOKEN, org=INFLUX_ORG)
    query_api = client.query_api()
    
    # Use same approach as fast training script - works reliably
    query = f'''
    ...
    print("⏳ Loading data from InfluxDB (sampling ~50k points)...")
    print("   This will take 1-2 minutes...")
    
    tables = query_api.query(query)
    
    # Convert to dataframe manually (more reliable than query_data_frame)
    data_rows = []
    for table in tables:
