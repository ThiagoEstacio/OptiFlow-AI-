...
@router.get("/insights/public", response_model=dict)
@cached(ttl=30, key_prefix="ai_insights_public")
async def get_insights_public(
    limit: int = Query(20, ge=1, le=100, description="Maximum insights to return"),
    severity: Optional[str] = Query(None, description="Filter by severity")
):
    """
    Get recent AI insights (public endpoint - no auth required)

    Returns the latest insights generated by the autonomous agent
    without requiring authentication. Perfect for dashboards and public displays.
    ...
    **Returns:**
    - List of recent insights with metadata
    """
    try:
        agent = get_autonomous_agent()

        if not agent:
            # If agent not initialized, return empty insights
            return {
                "total": 0,
                "insights": [],
                "status": "agent_not_initialized",
                "message": "Autonomous agent is initializing. Please try again in a moment."
            }
        ...
        logger.error(f"Error getting public insights: {e}")
...
@router.get("/insights/public/tag/{tag_id}", response_model=dict)
@cached(ttl=45, key_prefix="ai_insights_public_tag")
async def get_tag_insights_public(
    tag_id: str,
    duration: str = Query("1h", description="Duration: 1h, 6h, 12h, 24h"),
    db: AsyncSession = Depends(get_db)
):
    """
    Get AI insights for a specific tag (public endpoint - no auth required)

    Analyzes a tag and returns insights without requiring authentication.

    **Parameters:**
    - tag_id: UUID of the tag to analyze
    - duration: Time window for analysis (1h, 6h, 12h, 24h)

    **Returns:**
    - Tag analysis with statistics, anomalies, and recommendations
    """
    try:
        # Get tag
        result = await db.execute(select(Tag).where(Tag.id == tag_id))
        ...
        try:
            influx_data = await influxdb_service.query_tag_data(
                tag_id=tag_id,
                start=start_time,
                end=end_time
            )
            ...
            if influx_data:
                data_df = pd.DataFrame(influx_data)
                statistics = {
                    "count": len(data_df),
                    "mean": float(data_df['value'].mean()) if 'value' in data_df else None,
                    "min": float(data_df['value'].min()) if 'value' in data_df else None,
                    "max": float(data_df['value'].max()) if 'value' in data_df else None,
                    "stddev": float(data_df['value'].std()) if 'value' in data_df else None,
                    "latest_value": float(data_df['value'].iloc[-1]) if 'value' in data_df and len(data_df) > 0 else None
                }
            else:
                statistics = {
                    "count": 0,
                    ...
                    "max": None,
                    "stddev": None,
                    "latest_value": float(tag.last_value) if tag.last_value else None
                }
        except Exception as e:
        ...
        insight = {
            ...
            "timestamp": datetime.now().isoformat(),
            "duration": duration,
            "current_value": statistics.get("latest_value"),
        ...
        if statistics.get("stddev") and statistics.get("mean"):
            cv = (statistics["stddev"] / statistics["mean"]) * 100 if statistics["mean"] != 0 else 0
            if cv > 20:
                recommendations.append({
                    "type": "info",
                    "message": f"📊 Alta variabilidade detectada (CV: {cv:.1f}%) - Investigar causas"
                })
            elif cv < 2:
                recommendations.append({
                    "type": "success",
                    "message": f"✅ Processo muito estável (CV: {cv:.1f}%) - Bom desempenho"
                })

        if statistics.get("count", 0) == 0:
            recommendations.append({
                "type": "warning",
                "message": "⚠️ Sem dados disponíveis no período selecionado"
            })
        elif statistics.get("count", 0) > 0:
            recommendations.append({
                "type": "info",
                "message": f"📊 {statistics['count']} pontos de dados analisados"
            })
        ...
        logger.error(f"Error getting public tag insights: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/health/public", response_model=dict)
@cached(ttl=30, key_prefix="ai_health_public")
async def get_ai_health_public():
    """
    Get AI system health (public endpoint - no auth required)

    Returns the health status of the AI/ML system without authentication.
        ...
        health = {
            ...
            "components": {
                "autonomous_agent": {
                    ...
                    "status": "running" if agent else "not_initialized",
                    "insights_count": len(agent.insights) if agent else 0
                },
                "ai_insights_service": {
                    "available": ai_service is not None,
                    "status": "running" if ai_service else "not_initialized"
                }
            }
        }

        # Overall status
        all_ok = agent is not None and ai_service is not None
