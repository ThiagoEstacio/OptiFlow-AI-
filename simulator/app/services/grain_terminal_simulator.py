...
Microserviço independente que simula um terminal graneleiro para:
- Gerar dados realistas de processo e elétricos
- Servir como OPC-UA server para o Gateway conectar
- Testar IA/ML, análises de energia e detecção de falhas
...
O Gateway deve conectar via OPC-UA e ler as tags, então publicar no Kafka.
"""
...
class GrainTerminalSimulator:
    ...
    - Serve OPC-UA server para Gateway conectar
    - NÃO publica diretamente no Kafka
    """
    ...
    async def step_async(self, dt_s: float = 1.0):
        """
        Executa step de simulação (async version)
    ...
    def get_all_tags(self) -> Dict[str, Any]:
        """
        Retorna todas as tags do PLC virtual.

        O Gateway OPC-UA deve chamar este método periodicamente e publicar no Kafka.
        Simula namespace OPC-UA real.
        """
        tags = {}
